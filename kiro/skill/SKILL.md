...
**情况 A：有 wiki/ 目录（由 research-wiki 入库创建）**
1. 读取 `wiki/_index.md`，了解已有主题、资料量、处理状态
2. 读取 `CONTEXT.md`，理解研究问题
3. 向用户确认研究问题，然后**跳过步骤 2，直接进入步骤 3**（wiki 已完成证据采集）
...
- 产出 A1：直接在对话中回答，按 `skill/templates/answer_summary.md` 结构。**控制在 300 字以内，最多不超过 500 字。建议部分必须结合 CONTEXT.md 给出针对性建议。** A1 只在对话中输出，不保存文件。
- 产出 A2：证据链图谱，**必须严格按 `skill/templates/evidence_chain.md` 的表格格式**（证据节点表 + 关系表 + 强度摘要），保存到 `outputs/evidence_chain.md`。不要用自由格式替代。
- **不产出完整报告，不保存 answer_summary 文件。**

**报告模式**（用户明确说"写报告""出报告""生成报告"）：
- 产出完整报告，严格按 `skill/templates/simple_report.md` 结构，保存到 `outputs/`
- 产出 A1 摘要，按 `skill/templates/answer_summary.md` 结构，保存到 `outputs/answer_summary.md`。**同样控制在 300-500 字。**
- 同时产出证据链图谱，保存到 `outputs/evidence_chain.md`

**写作原则**：按"回答问题"组织，不按"数据来源"组织。每个发现/结论下面，用户数据、文献、竞品混在一起作为证据支撑，不要分别列"用户怎么说""文献怎么说""竞品怎么做"。结论在前，证据在后。

#### 报告内容

报告模式下，按 `skill/templates/simple_report.md` 结构产出完整报告（含侦探备忘录）。提问模式下只产出 A1 + A2。
...
五个核心侦探动作的详细执行指南见 `skill/guides/research_methodology.md`。

### 侦探工具箱（按需加载）

`skill/guides/detective_toolkit.md` 是完整的 26 个侦探分析工具，按四层组织：
