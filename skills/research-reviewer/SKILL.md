...
- 不是"每个章节的小结论"
- 不是"措辞最响的句子",是"承重墙"——其他次级判断挂在这条上面,这条塌了上面全塌
- **数量由报告本身决定,不预设**:多数报告 1-3 条;多课题综合报告 / 战略级报告可能更多——后者**分批审,每批不超过 3 条**(见效率规则)。每批 ≤3 不是凑数,是为了保证每条结论都能跑完"3 轮反面证据搜索"的对抗深度;一次塞 6 条会退化成浅扫
- **写明取舍**:在 review.md 开头列"本批审查的 N 条 / 总核心结论 K 条 / 为什么先审这 N 条"——让用户知道哪些被纳入、哪些被推迟、判断依据是什么
