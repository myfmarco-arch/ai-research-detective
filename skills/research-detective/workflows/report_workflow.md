...
9. **表达 profile**:是否需要加载特定报告风格。默认加载 [../guides/deep_report_style.md](../guides/deep_report_style.md) 作为深度分析报告叙事风格;若客户/公司/使用者有自定义表达逻辑,优先加载项目内独立风格文件。

如果用户只说"按你判断写"也要先复述你的理解,列出上述 9 项的默认假设,请用户确认或修改。用户确认前,不得写完整报告正文;最多只能给报告大纲/研究问题澄清草案。
...
[../guides/deep_report_style.md](../guides/deep_report_style.md):默认加载的深度分析报告叙事风格。它规定 AI 落笔流程、读者路径、结构节奏、开头、执行摘要、方法说明、主体发现、建议收束、标题规则、信息密度和交付判定。

若项目根或用户提供了公司 / 使用者专属的报告风格文件,把它作为本次表达 profile 加载;专属 profile 可覆盖本文件的叙事偏好,但不得覆盖事实、证据强度、可追溯性和 lint 红线。

### 2.4 写作原则

按"回答问题"组织,不按"数据来源"组织。每个发现/结论下面,用户数据、文献、竞品混在一起作为证据支撑,不要分别列"用户怎么说""文献怎么说""竞品怎么做"。结论在前,证据在后。

两份合起来构成报告的完整产出标准。若加载了表达 profile,落笔后还要对照 profile 的自检清单;profile 任一硬约束不过 → 改完再交付。

### 2.5 对照 CONTEXT 的项目级约束
