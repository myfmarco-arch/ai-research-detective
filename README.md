...
- 每次 reviewer 审查发现反例、推翻理论或补上盲区后，自动回写 wiki，标 `#review_YYYYMMDD` 来源
...
每个工具都带使用场景和操作步骤。detective 会按方法论选工具、按证据强度下判断，而不是只做通用总结。详见 [detective_toolkit.md](skills/research-detective/guides/detective_toolkit.md) 末尾速查表。
